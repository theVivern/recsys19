...
def process_test_naives_bayes(data, session_length: int):
    ...
    for i in range(session_length):
        data_wide=data_wide.join(meta.set_index('item_id'), on=('reference|' + str(i+1)),rsuffix = ('|' + str(i+1)))
    
    # add platform, device
    platform_device=data.loc[(data.key.isin(data_wide.index)) & (data.step_inverted==0),['key','platform','device','impressions','prices']]
